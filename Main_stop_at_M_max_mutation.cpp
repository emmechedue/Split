...
/********************		IMPORTANT:                  ****************************************************
      ...
      THIS PROGRAM DOES THE SAME THING THAT MUTATION DOES BUT THE CONDITION TO STOP THE LOOP IS NOT THE TIME BUT I STOP AS SOON AS I REACH THE MAXIMUM NUMBER OF GROUPS!!!!!
    ...
    for(iloop=0;iloop<cons.N_loop;iloop++){
		...
		do{ 
			...
			if(Nc[m]>0){ //Here I check if in the cell where the reaction just happened there still is a defector
				rand=gsl_rng_uniform (r); //This part here is the mutation
				if(rand<eps){
				Nc[m]--;
				Nd[m]++;
				}
