...
/********************* NOTE: This program does the same thing that main_cutted does but since I want to check that the Moran process predictions are correct, I am starting from a situation where I already have fixated groups and I choose which percentage of groups is cooperative and which is defective. */
//*****************************
...
int main(){
   ...
   //******let's take the seed for the rng and initialize the rng******
	...
	TI=fread (&seed, sizeof (seed), 1, pfile); //I added the TI= ... just to not be bothered anymore by the warnings!
	fclose(pfile);
    ...
    for(iloop=0;iloop<cons.N_loop;iloop++){
		...
		do{ 
			...
			if(check(Nc, Nd, cons, m)==true){ //Of course I need to check if I have to split the cell or not
		   		M=createcellmoran(M, m, Nc, Nd, x, Gamma, G, cons, r); 
